...
class DivSelector(Selector):
  """A class that selects <div> tags from a html page"""
  ...
  def find_descendants(self, path_to_descendant_tag):
    if self.name not in path_to_descendant_tag:
      raise IllegalArgumentError("The path string passed as argument does not contain a <div> tag as a parent")
