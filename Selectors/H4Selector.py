...
class H4Selector(Selector):
  """A class that selects a <h4> tags from a html page"""
  ...
  def find_descendants(self, path_to_descendant_tag):
    if self.name not in path_to_descendant_tag:
      raise IllegalArgumentError("The path string passed as argument does not contain a <h4> tag as a parent")
    else:
